...
describe('Service: CryptoLocalStorageService', function () {
  ...
  describe('using "localStorage"', function(){
    ...
    describe('init', function(){
      it('service should be initialyzeds with internal variables configurateds', function () {
        CryptoLocalStorageService.init({secret: secret});

        expect(CryptoLocalStorageService.JSON !== null).toBe(true);
        expect(CryptoLocalStorageService.storageType === 'localStorage').toBe(true);
  ...
  describe('using "sessionStorage"', function(){
    ...
    describe('init', function(){
      it('service should be initialyzeds with internal variables configurateds', function () {
        CryptoLocalStorageService.init({secret: secret, storageType: storageType});

        expect(CryptoLocalStorageService.JSON !== null).toBe(true);
        expect(CryptoLocalStorageService.storageType === storageType).toBe(true);
        expect(CryptoLocalStorageService.CryptoJS !== null).toBe(true);
        expect(CryptoLocalStorageService.secret !== '').toBe(true);
      });
    });
